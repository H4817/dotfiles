 ...
 */
        ...
        BRW_langLoaded(function(){
            ...
            getOptionsCurentTab(function() {
                ...
                setVisibleElementsBySortType(
                    localStorage.getItem('themes-sort-type') || 2
                    ,
                    localStorage.getItem('settings-background-current-tab') || 0
                );
            });
        });
    });
    ...
    function getAvailableThemes(response, callback) {
        ...
        if(availableThemesData) {
            ...
            if(getImagesSortType() == 0){//gallery
                ...
                for(i in themesList["static_backgrounds_user"])
                    addAvailableContentToList($staticBackgrounds, themesList["static_backgrounds_user"][i], staticBackgroundType, currentContent);
            }
                        ...
                        if(Object.keys(themes).length > 1){//Delete theme
                            if(themes[contentId]){
                                if(themes[contentId].downloadedByUser){//delete user theme
                                    ...
                                    for(var u in uploadedThemes) if(uploadedThemes[u].id == contentId){
    ...
    function addImagesUploadButtonHandler() {
        ...
        function changeTitleFromFileName(obj){
            ...
            if(src && $uploadDialogTitle.attr("touched") != 'yes'){
                $uploadDialogTitle.val(clearFileNameForTitle(src));
            }
        }
    ...
    function addImagesSortButtonHandler() {
        ...
        var $el = $(".options-settings-images-sort-item");
        ...
        $el.on("click", function() {
            ...
            if(currentSort != val){
                BRW_sendMessage({command: "setImagesSortType", val: val});
            }//if
        });
    }
    /**
        ...
        if(tab == 0){//flixels tab
            switch(parseInt(val)){
                ...
                case 4://search themes
                    ...
                    $('#themes-search-form-wrap').css({display:'block'});
                    var srch = new themesSearch(); srch.init();
    ...
    function blinkActiveSortType(){
        var $actives = $(".options-settings-themes-sort-item, .options-settings-images-sort-item").filter('.active');
        var color = {
            's' : "#449d44",
            'b' : "#b2fe08"//"#4CE6DD"
        };
        ...
        $actives.each(function(){
            $(this)
                .stop(true, true).css({"background-color":color.s})
                .animate({"opacity":1}, 500)
                .animate({"background-color":color.b}, speed).animate({"background-color":color.s}, speed)
                .animate({"background-color":color.b}, speed).animate({"background-color":color.s}, speed)
                .animate({"background-color":color.b}, speed).animate({"background-color":color.s}, speed)
            ;
        });
     ...
     */
    function addSelectedTxt($tabButton){
        //$(".settings-background-tab-button .selectedTxt").remove();
        $tabButton.find("span").append($(".selectedTxt"));
    }
    ...
    function updateAvailableThemesListOnSearch(query) {
        ...
        if(query.length){
            BRW_sendMessage({command: "updateAvailableThemesList", search:query}, function(availableThemesResponse){
                BRW_sendMessage({command: "updateFlixelThemesList"}, function(){
                    getAvailableThemes(availableThemesResponse, displayAvailableThemesList); 
                }); 
            });
...
function themesSearch(data){
    ...
    this.handleSubmit = function(dropdown, text){
        ...
        if(query.length){
            updateAvailableThemesListOnSearch(String(query).trim());
            myself.list.css("display","none");
