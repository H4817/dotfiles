...
function bgAddNewGroup(group, collectGroups, sendResponse) {
     BRW_bgAddNewGroup(group, collectGroups, sendResponse);
}
...
function bgEditNewGroup(group, collectGroups, sendResponse) {
    ...
    BRW_dbTransaction(function(fx){
            ...
            if(id && title) { 
                ...
                /*
                tx.executeSql('UPDATE GROUPS SET title = ? WHERE id = ?', [
                    group.title,
                    group.id
                ], function() {
                    sendResponse({"group" : group});
    ...
    if(groupId) {
        ...
        /*
                tx.executeSql('DELETE FROM GROUPS WHERE id = ?', [groupId], function() {
                    tx.executeSql('DELETE FROM DIALS WHERE groupId = ?', [groupId]);
                });
                bgMoveGroupsOrder(groups);
        */
    }
...
function bgGetGroupDialsCount(groupId, sendResponse) {
    if(groupId) {
        BRW_dbTransaction(function(tx) {
                ...
                /*
                tx.executeSql('SELECT COUNT(id) AS dialsCount FROM DIALS WHERE groupId = ?', [groupId], function (tx, results) {
                    var countItems = results.rows;
                    var countItemsLength = countItems.length;
                    var dialsCount = countItemsLength && countItems[0].dialsCount ? parseInt(countItems[0].dialsCount) : 0;
                    sendResponse({"dialsCount" : dialsCount});
 ...
 * @param collectGroups Array
 */
function bgMoveGroupsOrder(collectGroups) {
        ...
        BRW_dbTransaction(function (tx) {
                for(var i in groups) {
                    if(groups.hasOwnProperty(i)) {
                        ...
                        /*
                        tx.executeSql('UPDATE GROUPS SET orderNum = ? WHERE id = ?', [i, groups[i]]);
                        */
                    }
                }
            });
...
function bgGetDefaultGroupData(withDials, sendResponse) {
    ...
    getDb().transaction(function (tx) {
            tx.executeSql('SELECT * FROM GROUPS WHERE type = ? LIMIT 1', [GROUP_TYPE_DEFAULT], function (tx, results) {
                if(withDials)
                    bgFindGroupDials(results.rows, sendResponse);
                else
                    sendResponse({"group": results.rows.length ? results.rows[0] : null});
...
function bgGetGroupData(groupId, withDials, sendResponse) {
    ...
    if(findGroup == GROUP_POPULAR_ID) {
        ...
        /*
        getDb().transaction(function (tx) {
            tx.executeSql('SELECT * FROM GROUPS WHERE id = ? LIMIT 1', [findGroup], function (tx, results) {
...
function bgGetAvailableGroups(groupId, sendResponse, sortType) {
    ...
    /*
    getDb().transaction(function (tx) {
            tx.executeSql('SELECT * FROM GROUPS ORDER BY orderNum ASC', [], function (tx, results) {
                var groups = [];
                if(results.rows.length) {
                    var skipActiveCheck = false;
                    for(var i in results.rows) {
