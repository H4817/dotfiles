        ...
        if (e.keyCode == 27) { // press esc
            ...
            if(enterChar.trim()) {
                ...
                if(query) {
                    ...
                    /*
                    googleSearchXhr = $.ajax({
                        type: "GET",
                        url: "http://google.com/complete/search",
                        data: "output=toolbar&q=" + encodeURIComponent(query),
                        success: function() {
                            var searchResponse = googleSearchXhr.responseXML;
                            if (searchResponse) {
                                var elements = searchResponse.getElementsByTagName("suggestion");
    ...
    function addSearchItemToList(searchQuery, $searchContainer, itemVal, isSiteUrl) { 
        isSiteUrl = false;
        var searchGoogleImgUrl = extensionGetUrl("pages/newtab/css/img/buttons/tiles/search.png");
        ...
        $searchBarItem.attr("data-search-url", isSiteUrl ? addDataUrlHttp(itemVal) : getRedirectSearchUrl(itemVal));
