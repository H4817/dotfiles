...
*/
    ...
    function addAvailableContentToList($container, theme, contentType, currentContent) {
        ...
        if(contentType == liveBackgroundType) {
            ...
            $contentResolutionsButtons = $("<div></div>");
            $contentResolutionsButtons.attr("data-content-type", liveThemesType);
            ...
            if(contentType == liveThemesType && !contentActive) {
                ...
                if(!contentActive) {
                    if(!contentDownloading) {
                        ...
                        if(contentType == liveBackgroundType || contentType == flixelBackgroundType) {
                            $("#available-favorite-themes").find(".av-content-resolution-buttons").hide();
                            $("#available-favorite-themes").find(".av-content-install[data-content-theme!='" + contentTheme + "']").filter("[data-content-type=" + contentType + "]").each(function() {
                                $resolutionButtons = $(this).parent().parent().find(".av-content-resolution-buttons");
                                $resolutionButtons.find("button").each(function() {
                                    var $button = $(this);
                                    if($button.hasClass("btn-success")) {
                                        $button.attr("is-active-resolution", false);
                            ...
                            BRW_sendMessage({command: "changeFlixerVideoBackground", theme: contentTheme});
                            setDownloadingButtonState($el, contentType);
                        }
                    }
                } else {
                    if(contentType == liveBackgroundType || contentType == flixelBackgroundType) {
                        $("#available-favorite-themes").find(".av-content-resolution-buttons").hide();
                        $el.parent().parent().find(".av-content-resolution-buttons").show();
                        clearSelectQualityButtonText(installContentText, selectQualityText);
                    }
                }
            }
