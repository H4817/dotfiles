...
var windowHeight, windowWidth, todoShowTimer;
...
function readTodoItems(callback, response) { 
    ...
    getDb().readTransaction(function (tx) {
        tx.executeSql('SELECT * FROM TODO_ITEMS ORDER BY item_order', [], function (tx, results) {
            var itemsLength = results.rows.length;
            response.items = [];
            for(var i = 0; i < itemsLength; i++) {
                response.items.push(results.rows[i]);
        ...
        //chrome.runtime.sendMessage({command: "clearDoneTodoElementsDb"});//BRW_sendMessage
    });
    */                   
}
...
function addTodoListDraggableProperty() { 
    $("#todo-container").draggable({ handle: "#todo-header", containment: "#background-borders", scroll: false,
