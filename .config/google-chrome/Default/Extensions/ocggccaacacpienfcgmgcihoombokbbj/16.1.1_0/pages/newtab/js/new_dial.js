 ...
 * @param group Object
 */
function displayGroupDials(group) { 
    group = prepareCurrentGroupTilesEnd(group);
    displayHistoryItems({dials : group.dials, group : group, displaySpeedDialPanel : true});
}
        ...
        BRW_sendMessage({
            ...
            if(data && data.dial) {
                ...
                BRW_sendMessage({command: "getActiveGroup", withDials: false}, function(data) {
                    ...
                    setTimeout(function() {
                        BRW_sendMessage({"command": "thumbLoad", "tiles" : [dial]});
                    }, 100);
                });
            }
        });
